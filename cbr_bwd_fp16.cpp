/**
 * @file test_cbr_bwd_fp16.cpp
 * @brief cuDNN Frontend Conv+BN+ReLU反向融合性能测试V1.1 (FP16版本，仅反向传播，支持Mode C穷举式搜索)
 * @version 1.1.0
 * @date 2026-04-13
 * @author 技术觉醒团队
 ...
 * @note 所属系列: cuda
 *
 * @note V1.1 改动（基于V1.0，引入ta_v4_common_fp16.hpp公共基础设施）：
 *   1. 【引入】ta_v4_common_fp16.hpp作为单一真理源
 *   2. 【删除】所有在ta_v4_common_fp16.hpp中已有的定义
 *   3. 【保留】反向传播核心实现（BN Backward→DGrad+WGrad）
 *   4. 【保留】三流架构和L2缓存优化策略
 *   5. 【保留】CUDA Graph模式支持
 *
 * @note 基于test_cbr_fp16_merged.cpp V9.3，删除所有前向传播代码，仅保留反向传播
 * @note 反向拓扑结构完全不变：
 *       反向拓扑：
 ...
 *                    │                                                            │
 *                    ↓                                                            ↓
 *                   S2 ──→ [DGrad] ──event_bwd_join_s2──┐              S3 ──→ [WGrad] ──event_bwd_join_s3──┐
 *                                                          │                                                  │
 *                                                          └──────────────→ (Both must complete) ←──────────────┘
 *
 * @note 关键优化:
...
public:
    ConvBNReLUBackwardBenchmark(const Config& config)
        ...
        std::cout << "Conv Backward Type: BN+DGrad+WGrad" << std::endl;
...
public:
    ...
    void capture_cuda_graphs() {
        ...
        CHECK_CUDA(cudaStreamWaitEvent(stream_comp_1_, event_bwd_join_s3_, 0));
