/**
 * @file fc.cpp
 * @brief cuDNN Frontend 全连接层性能测试 (FP16版本，使用1x1卷积实现)
 * @version 3.0.0
 * @date 2026-04-18
 * @author 技术觉醒团队
 ...
 *   5. 【关键】所有输入输出都是 FP16，内部计算（累加器）用 FP32
 *   6. 虚张量优化：卷积输出设为虚张量，显存带宽降低 40%
 *   7. 独立 CUDA Graph：正向反向分别捕获、分开计时
 *   8. 兼容 ResNet-50 末端：自动适配 GAP 输出(512,1,1,2048)
 *   9. 【新】偏置梯度使用 cuDNN Frontend Reduction API，不使用自定义 .cu 文件
 *
 * @note 测试命令：
 *   ./fc --batch_size 512 --in_features 2048 --out_features 1000 --bias True --mode B --graph true
 */
...
// ████████████████████████████████████████████████████████████████████████████
// ████ 偏置梯度计算Kernel声明（在fc_bias_gradient.cu中实现） ████
// ████████████████████████████████████████████████████████████████████████████
...
inline FCConfig parse_fc_arguments(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--batch_size" && i + 1 < argc) {
            ...
            std::cout << std::endl;
            std::cout << "ResNet-50 FC Test:" << std::endl;
...
private:
    ...
    void *d_B_;        // 偏置 (C_out_aligned) [FP16]
    ...
    void *d_dB_;       // 偏置梯度 (C_out_aligned) [FP16]
...
private:
    ...
    void allocate_memory() {
        ...
        const size_t B_size = C_out_aligned_ * sizeof(__half);
        ...
        if (config_.use_bias) {
            d_B_ = allocate_aligned_gpu_memory(B_size);
        }

        // 反向传播内存（全部FP16）
        d_dY_ = allocate_aligned_gpu_memory(Y_size);
        d_dX_ = allocate_aligned_gpu_memory(X_size);
        d_dW_ = allocate_aligned_gpu_memory(W_size);  // 【关键】dW 也是 FP16

        if (config_.use_bias) {
            d_dB_ = allocate_aligned_gpu_memory(B_size);  // 【关键】dB 也是 FP16
        }
    }
    ...
    void initialize_data() {
        ...
        if (config_.use_bias) {
            initialize_random_fp16(d_B_, C_out_, 45);
        }
    }
...
public:
    ...
    void warmup(int iterations = 50) {
        ...
        for (int i = 0; i < iterations; ++i) {
            ...
            if (config_.use_bias) {
                compute_bias_gradient_fp16(static_cast<const __half*>(d_dY_),
                                     static_cast<__half*>(d_dB_),
                                     static_cast<int>(N_),
...
int main(int argc, char** argv) {
    std::cout << "=== 技术觉醒 V4 全连接层高性能测试 ===" << std::endl;
