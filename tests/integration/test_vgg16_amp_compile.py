...
def build_model_optimizer_scheduler(device, local_rank):
    """Build model, DDP wrap, optimizer, scheduler and AMP scaler."""
...
def main():
    ...
    model, criterion, optimizer, scheduler, scaler = build_model_optimizer_scheduler(device, local_rank)

    # ====================================================================
    # Warmup: trigger torch.compile(max-autotune) so compilation finishes
    ...
    dummy_data = torch.randn(
        LOCAL_BATCH_SIZE, 3, 224, 224, device=device
    ).to(memory_format=torch.channels_last)
    dummy_target = torch.randint(0, 1000, (LOCAL_BATCH_SIZE,), device=device)

    model.train()
    optimizer.zero_grad()
    with autocast('cuda'):
        out = model(dummy_data)
        l = criterion(out, dummy_target)
    ...
    if is_main:
        ...
        log("=====================================")
        log(f" Mode:       GPU [AMP] (torch.compile)")
    ...
    if is_main:
        log("")
        log("=====================================")
        log(" Mode:       GPU [AMP] (torch.compile, DDP)")
        log("=====================================")
