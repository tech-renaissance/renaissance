...
int main(int argc, char* argv[]) {
    ...
    if (dataset_type_str == "mnist") {
        ...
        std::cerr << "Supported: mnist, cifar10\n";
        return 1;
    }
    ...
    std::cout << "\nPath: " << dataset_path << "\n";
    prep.config_dataset(dataset_type, use_dts, 0);
    ...
    if (device_type == "GPU" && !gpu_ids_str.empty()) {
        prep.config_device(device_type, gpu_ids_str, false);
    } else {
        prep.config_device(device_type, false);
    }

    // 设置current_resolution（必须在multi_thread_init之前设置）
    // 因为multi_thread_init可能会触发begin_train导致is_busy=true
    ...
    std::unique_ptr<PreprocessOperation> train_po1;

    if (is_resize_or_crop_po(po_train1)) {
        train_po1 = create_resize_crop_po(po_train1, resolution, scale_min, scale_max,
                                          ratio_min, ratio_max);
    } else {
        train_po1 = create_po(po_train1, scale_min, scale_max,
    ...
    if (!po_train2.empty()) {
        ...
        if (is_resize_or_crop_po(po_train2)) {
            train_po2 = create_resize_crop_po(po_train2, resolution, scale_min, scale_max,
                                              ratio_min, ratio_max);
        } else {
            train_po2 = create_po(po_train2, scale_min, scale_max,
                                  ratio_min, ratio_max, flip_prob);
        }
        ...
        if (!po_train3.empty()) {
            ...
            std::unique_ptr<PreprocessOperation> train_po3;

            if (is_resize_or_crop_po(po_train3)) {
                train_po3 = create_resize_crop_po(po_train3, resolution, scale_min, scale_max,
                                                  ratio_min, ratio_max);
            } else {
                train_po3 = create_po(po_train3, scale_min, scale_max,
                                      ratio_min, ratio_max, flip_prob);
            }
            if (!train_po3) return 1;
            prep.set_train_transforms(*train_po1, *train_po2, *train_po3);
        } else {
            prep.set_train_transforms(*train_po1, *train_po2);
        }
    } else if (!po_train3.empty()) {
        std::cout << "Train PO 3: " << po_train3 << "\n";
        std::unique_ptr<PreprocessOperation> train_po3;
        ...
        if (!train_po3) return 1;
        prep.set_train_transforms(*train_po1, *train_po3);
    } else {
        prep.set_train_transforms(*train_po1);
    }

    std::cout << "Val PO 1: " << po_val1 << "\n";
    std::unique_ptr<PreprocessOperation> val_po1;

    if (is_resize_or_crop_po(po_val1)) {
        val_po1 = create_resize_crop_po(po_val1, resolution, scale_min, scale_max,
                                        ratio_min, ratio_max);
    } else {
        val_po1 = create_po(po_val1, scale_min, scale_max,
                            ratio_min, ratio_max, flip_prob);
    }
    if (!val_po1) return 1;
    ...
    if (!po_val2.empty()) {
        ...
        if (is_resize_or_crop_po(po_val2)) {
            val_po2 = create_resize_crop_po(po_val2, resolution, scale_min, scale_max,
                                            ratio_min, ratio_max);
        } else {
            val_po2 = create_po(po_val2, scale_min, scale_max,
                                ratio_min, ratio_max, flip_prob);
        }
        if (!val_po2) return 1;
        ...
        if (!po_val3.empty()) {
            ...
            std::unique_ptr<PreprocessOperation> val_po3;

            if (is_resize_or_crop_po(po_val3)) {
                val_po3 = create_resize_crop_po(po_val3, resolution, scale_min, scale_max,
                                                ratio_min, ratio_max);
            } else {
                val_po3 = create_po(po_val3, scale_min, scale_max,
                                    ratio_min, ratio_max, flip_prob);
            }
            if (!val_po3) return 1;
            prep.set_val_transforms(*val_po1, *val_po2, *val_po3);
        } else {
            prep.set_val_transforms(*val_po1, *val_po2);
        }
    } else if (!po_val3.empty()) {
        std::cout << "Val PO 3: " << po_val3 << "\n";
        std::unique_ptr<PreprocessOperation> val_po3;
        ...
        if (!val_po3) return 1;
        prep.set_val_transforms(*val_po1, *val_po3);
    } else {
    ...
    // 【MSYS2兼容性修复】显式清理并直接退出，避免atexit handler在MSYS2环境下崩溃
    // 在MSYS2环境中，mimalloc的atexit handler可能与ucrtbase冲突
    // 通过调用_Exit直接终止进程，跳过atexit handler
    std::cout.flush();
    _Exit(0);  // 使用_Exit而不是exit，跳过atexit handler
