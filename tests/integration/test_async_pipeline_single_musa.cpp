/**
 * @file test_async_pipeline_single_musa.cpp
 * @brief 单MUSA GPU异步流水线集成测试
 * @version 3.6.18
 * @date 2026-01-03
 * @author 技术觉醒团队
 */
...
int main() {
    std::cout << "========================================" << std::endl;
    std::cout << "  Single MUSA GPU Async Pipeline Test"  << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    try {
        ...
        auto pinned = musa.alloc_pinned(num_bytes);
        ...
        // ============================================================
        // 步骤3：创建MUSA GPU tensor
        // ============================================================
        std::cout << "\n[Step 3] Creating MUSA GPU tensor..." << std::endl;
        Tensor device_tensor = musa.empty(shape, DType::FP32);
        std::cout << "  SUCCESS: MUSA GPU tensor created" << std::endl;
        ...
        // 模拟计算：将所有值乘以2（通过自加实现）
        Tensor result = musa.empty(shape, DType::FP32);
        ...
        musa.transfer_into(result, cpu_verify);
...
#else // TR_USE_MUSA not defined

#include <iostream>

int main() {
    std::cerr << "ERROR: This test requires MUSA support (TR_USE_MUSA not defined)" << std::endl;
    return 1;
}
