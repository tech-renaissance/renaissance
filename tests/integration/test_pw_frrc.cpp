/**
 * @file test_pw_frrc.cpp
 * @brief 测试PW的FastRandomResizedCrop功能（测试模式，SDMP=1）
 * @version 1.0.0
 * @date 2026-02-23
 * @author 技术觉醒团队
 */
...
void print_usage(const char* program_name) {
    std::cout << "Usage: " << program_name << " [OPTIONS]\n\n"
              ...
              << "  --scale-min <F>      Min scale for FastRandomResizedCrop (default: 0.08)\n"
              << "  --scale-max <F>      Max scale for FastRandomResizedCrop (default: 1.0)\n"
              << "  --ratio-min <F>      Min ratio for FastRandomResizedCrop (default: 0.75)\n"
              << "  --ratio-max <F>      Max ratio for FastRandomResizedCrop (default: 1.333)\n"
              ...
              << "      SDMP factor is fixed to 1 (no multi-augmentation)\n";
}
...
int main(int argc, char* argv[]) {
    ...
    // 步骤4：设置数据变换（FastRandomResizedCrop）
    std::cout << "Setting transforms (FastRandomResizedCrop)...\n";
    std::cout << "  Scale range: [" << scale_min << ", " << scale_max << "]\n";
    std::cout << "  Ratio range: [" << ratio_min << ", " << ratio_max << "]\n";
    std::cout << "  Random seed: " << random_seed << "\n";
    prep.set_train_transforms(FastRandomResizedCrop(resolution, scale_min, scale_max,
                                                     ratio_min, ratio_max));
    prep.set_val_transforms(FastRandomResizedCrop(resolution, scale_min, scale_max,
                                                   ratio_min, ratio_max));
