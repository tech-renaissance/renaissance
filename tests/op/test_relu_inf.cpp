/**
 * @file test_relu_inf.cpp
 * @brief ReLU INF 算子测试 — 支持 CPU / GPU / AMP 三种模式
 * @version 1.0.0
 * @date 2026-05-17
 * @author 技术觉醒团队
 *
 * 用法：
 *   test_relu_inf.exe --cpu  [--shape 8,1024,1024,8]
 *   test_relu_inf.exe --gpu  [--shape 8,1024,1024,8]
 *   test_relu_inf.exe --amp  [--shape 8,1024,1024,8]
 *
 * 注意：
 ...
 *
 * 关键特性：
 *   - 测试推理算子（RELU_FP32_INF / RELU_AMP_INF）
 *   - 虽然分配bitmask张量，但不需要验证其值（推理算子不写bitmask）
 *   - 不测试BWD算子，只测试前向推理
 */
...
int main(int argc, char** argv) {
    ...
    std::string ws = std::string(TR_WORKSPACE) + "/relu_inf_data";
    ...
    py << "python ";
#endif
    py << std::string(TR_PROJECT_ROOT) << "/tests/op/test_relu_inf.py"
       << " --shape " << cfg.shape_str
       << " --seed " << cfg.seed
       << " --workspace \"" << ws << "\""
       << " --dtype " << py_dtype;
    ...
    Tensor h_x  = Tensor::load_tensor(ws + "/x_inf"     + tsr_sfx + ".tsr");
    Tensor h_y  = Tensor::load_tensor(ws + "/y_ref_inf" + tsr_sfx + ".tsr");
    ...
    // 注意：推理算子仍然需要分配bitmask张量，保持接口一致性
    // 但推理算子不会往bitmask张量里写数据，所以这里不需要验证mask的值
    DTensor d_mask = task.alloc(shape, DType::INT8, Region::S_MASK);
    task.finalize_memory();

    ComputationGraph g;
    if (is_amp) {
        g.append(ComputeOp::RELU_AMP_INF, {d_x.id}, {d_y.id, d_mask.id});
    } else {
        g.append(ComputeOp::RELU_FP32_INF, {d_x.id}, {d_y.id, d_mask.id});
    }
    task.add_graph("inf", std::move(g), StreamKind::COMP_1);
    ...
    for (int rank = 0; rank < num_ranks; ++rank) {
        ...
        // 注意：我们不验证mask的值，因为推理算子不会往里面写数据
        // 这是推理算子和训练算子的关键区别
    }

    std::cout << "\n===== ReLU INF [" << mode_name(cfg.mode) << "] ("
              << num_ranks << " rank(s)): "
              << (all_pass ? "PASS" : "FAIL") << " =====\n"
