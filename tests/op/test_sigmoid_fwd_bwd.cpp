/**
 * @file test_sigmoid_fwd_bwd.cpp
 * @brief Sigmoid FWD+BWD 串接测试 — 支持 CPU / GPU / AMP 三种模式
 * @version 1.0.0
 * @date 2026-06-01
 * @author 技术觉醒团队
 *
 * 用法：
 *   test_sigmoid_fwd_bwd.exe --cpu  [--shape 8,1024,1024,8]
 *   test_sigmoid_fwd_bwd.exe --gpu  [--shape 8,1024,1024,8]
 *   test_sigmoid_fwd_bwd.exe --amp  [--shape 8,1024,1024,8]
 *
 * 注意：
...
int main(int argc, char** argv) {
    ...
    std::string ws = std::string(TR_WORKSPACE) + "/sigmoid_fwd_bwd_data";
    ...
    py << "python ";
#endif
    py << std::string(TR_PROJECT_ROOT) + "/tests/op/test_sigmoid_fwd_bwd.py"
       << " --shape " << cfg.shape_str
       << " --seed " << cfg.seed
       << " --workspace \"" << ws << "\""
       << " --dtype " << py_dtype;
    ...
    ComputationGraph g;
    ComputeOp fwd_op = is_amp ? ComputeOp::SIGMOID_AMP_FWD : ComputeOp::SIGMOID_FP32_FWD;
    ComputeOp bwd_op = is_amp ? ComputeOp::SIGMOID_AMP_BWD : ComputeOp::SIGMOID_FP32_BWD;
    ...
    std::cout << "\n===== Sigmoid FWD+BWD " << mode_name(cfg.mode)
              << " (" << num_ranks << " rank(s)): "
              << (all_pass ? "PASS" : "FAIL") << " =====\n"
