 ...
 *   INF 模式不计算 GenStats，sum/sq_sum 不被填充，
 *   但作为统一接口的 outputs 需要保留占位。
 *
 * 关键特性：
...
int main(int argc, char** argv) {
    ...
    DTensor d_sum    = task.alloc(Shape{1, 1, 1, cfg.K}, DType::FP32, Region::T_TEMP_FP32);  // sum (reserved)
    DTensor d_sq_sum = task.alloc(Shape{1, 1, 1, cfg.K}, DType::FP32, Region::T_TEMP_FP32);  // sq_sum (reserved)
    ...
    // INF: {X, W} -> {Y, sum, sq_sum}（sum/sq_sum 为预留输出，INF 不填充但保留接口）
    g_inf.append(inf_op, {d_x.id, d_w.id},
