#!/usr/bin/env python3
"""
ELU FWD+BWD 串接测试 — PyTorch 参考数据生成
FWD:  y = x > 0 ? x : exp(x) - 1  (alpha = 1.0)
BWD:  dx = dy * (x > 0 ? 1 : exp(x))
支持 --dtype fp16 | fp32
"""
...
def generate(shape: tuple, seed: int, ws: str, dtype: str):
    ...
    Y = torch.where(X > 0, X, torch.exp(X) - alpha)

    # ── BWD ──
    torch.manual_seed(seed + 1000)
    dY = torch.randn(N, H, W, C, dtype=torch_dtype) * 2.0
    grad = torch.where(X > 0, torch.ones_like(X), torch.exp(X))
    dX = dY * grad
    ...
    print(f"ELU FWD+BWD {dtype_label} reference generated in: {ws}")

def main():
    parser = argparse.ArgumentParser(description='ELU FWD+BWD test data generator')
