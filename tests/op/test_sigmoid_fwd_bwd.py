#!/usr/bin/env python3
"""
Sigmoid FWD+BWD 串接测试 — PyTorch 参考数据生成
FWD:  y = sigmoid(x) = 1 / (1 + exp(-x))
BWD:  dx = dy * sigmoid(x) * (1 - sigmoid(x))
支持 --dtype fp16 | fp32
"""
...
def generate(shape: tuple, seed: int, ws: str, dtype: str):
    ...
    print(f"Sigmoid FWD+BWD {dtype_label} reference generated in: {ws}")

def main():
    parser = argparse.ArgumentParser(description='Sigmoid FWD+BWD test data generator')
