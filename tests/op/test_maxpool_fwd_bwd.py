#!/usr/bin/env python3
"""
MaxPool FWD+BWD 串接测试 — PyTorch 参考数据生成
FWD:  y = maxpool2d(x)
BWD:  dx = maxpool2d_backward(dy, y, x)
输出: FWD 的输入 x、输出 y、BWD 的输入 dy、输出 dx。不输出 mask TSR。
支持 --dtype fp16 | fp32。
"""
...
def generate(shape: tuple, kernel: int, stride: int, padding: int, seed: int, ws: str, dtype: str):
    ...
    # PyTorch 使用 NCHW 格式，需要转换
    # FWD:  y = maxpool2d(x)
    X = torch.randn(N, C, H, W, dtype=torch_dtype) * 2.0
    pool = torch.nn.MaxPool2d(kernel_size=kernel, stride=stride, padding=padding)
    Y = pool(X)
    ...
    print(f"MaxPool FWD+BWD {dtype_label} reference generated in: {ws}")
    print(f"  Input shape: (N,C,H,W)=({N},{C},{H},{W}) -> Output: (N,C,OH,OW)=({N},{C},{OH},{OW})")

def main():
    parser = argparse.ArgumentParser(description='MaxPool FWD+BWD test data generator')
    parser.add_argument('--shape', default='8,224,224,64',
                        help='NHWC shape (comma-separated, e.g. 8,224,224,64)')
