/**
 * @file test_relu6_fwd_bwd.cpp
 * @brief ReLU6 FWD+BWD 串接测试 — 支持 CPU / GPU / AMP 三种模式
 * @version 4.20.1
 * @date 2026-06-28
 * @author 技术觉醒团队
 * @note 所属系列: tests/op
 *
 * 用法：
 *   test_relu6_fwd_bwd.exe --cpu  [--shape 8,1024,1024,8]
 *   test_relu6_fwd_bwd.exe --gpu  [--shape 8,1024,1024,8]
 *   test_relu6_fwd_bwd.exe --amp  [--shape 8,1024,1024,8]
 *
 * 注意：
...
int main(int argc, char** argv) {
    ...
    std::string ws = std::string(TR_WORKSPACE) + "/relu6_fwd_bwd_data";
    ...
    py << "python ";
#endif
    py << std::string(TR_PROJECT_ROOT) + "/tests/op/test_relu6_fwd_bwd.py"
       << " --shape " << cfg.shape_str
       << " --seed " << cfg.seed
       << " --workspace \"" << ws << "\""
       << " --dtype " << py_dtype;
    ...
    ComputationGraph g;
    ComputeOp fwd_op = is_amp ? ComputeOp::RELU6_AMP_FWD : ComputeOp::RELU6_FP32_FWD;
    ComputeOp bwd_op = is_amp ? ComputeOp::RELU6_AMP_BWD : ComputeOp::RELU6_FP32_BWD;
    ...
    std::cout << "\n===== ReLU6 FWD+BWD " << mode_name(cfg.mode)
              << " (" << num_ranks << " rank(s)): "
              << (all_pass ? "PASS" : "FAIL") << " =====\n"
