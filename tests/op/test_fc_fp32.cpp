...
int main() {
    ...
    std::cout << "===== FC FP32 性能测试 =====" << std::endl;
    std::cout << "配置: Batch=" << batch << ", In=" << in_features
              << ", Out=" << out_features << ", Bias=" << (has_bias ? "ON" : "OFF") << std::endl;
    std::cout << "GPU: FP32, 预热=" << warmup << "次, 计时=" << iterations << "次" << std::endl;
    std::cout << std::endl;
    ...
    DTensor d_x  = task.alloc({batch, 1, 1, in_features}, dtype, Region::F_FEATURE_FP32);          // 输入
    DTensor d_w  = task.alloc({out_features, 1, 1, in_features}, dtype, Region::W_FC_WEIGHT);   // 权重
    DTensor d_b  = task.alloc({1, 1, 1, out_features}, DType::FP32, Region::W_FC_BIAS);     // bias
    DTensor d_y  = task.alloc({batch, 1, 1, out_features}, dtype, Region::F_FEATURE_FP32);         // 前向输出
    DTensor d_dy = task.alloc({batch, 1, 1, out_features}, dtype, Region::F_FEATURE_FP32);         // 反向输入
    DTensor d_dx = task.alloc({batch, 1, 1, in_features}, dtype, Region::F_FEATURE_FP32);          // 反向输出
    DTensor d_dw = task.alloc({out_features, 1, 1, in_features}, DType::FP32, Region::G_FC_WEIGHT);  // 梯度
    ...
    (void)d_mb; (void)d_vb; (void)d_mw; (void)d_vw;

    task.finalize_memory();

    // 初始化张量数据（使用Tensor::randn方法）
    Tensor h_x = Tensor::randn({batch, 1, 1, in_features}, DType::FP32, 0.0f, 0.1f);
    Tensor h_w = Tensor::randn({out_features, 1, 1, in_features}, DType::FP32, 0.0f, 0.1f);
    Tensor h_b = Tensor::randn({1, 1, 1, out_features}, DType::FP32, 0.0f, 0.01f);
    Tensor h_dy = Tensor::randn({batch, 1, 1, out_features}, DType::FP32, 0.0f, 0.2f);
    ...
    g_fwd.append(ComputeOp::FC_FP32_FWD, {d_x.id, d_w.id, d_b.id}, {d_y.id}, op_params);
    task.add_graph("fwd", std::move(g_fwd), StreamKind::COMP_1);

    // ===== 构建BWD图 =====
    ComputationGraph g_bwd;
    g_bwd.append(ComputeOp::FC_FP32_BWD,
    ...
    // ===== 测试FWD性能 =====
    std::cout << "===== FC FP32 FWD 性能测试 =====" << std::endl;
