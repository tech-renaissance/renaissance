#!/usr/bin/env python3
"""
LeakyReLU FWD+BWD 串接测试 — PyTorch 参考数据生成
FWD:  y = x > 0 ? x : 0.01 * x
BWD:  dx = dy * (x > 0 ? 1 : 0.01)
支持 --dtype fp16 | fp32
"""
...
def generate(shape: tuple, seed: int, ws: str, dtype: str):
    ...
    Y = torch.where(X > 0, X, neg_slope * X)

    # ── BWD ──
    torch.manual_seed(seed + 1000)
    dY = torch.randn(N, H, W, C, dtype=torch_dtype) * 2.0
    slope = torch.where(X > 0, torch.ones_like(X), torch.full_like(X, neg_slope))
    dX = dY * slope
    ...
    yaml = (
        f"op: leaky_relu_fwd_bwd\n"
        f"dtype: {dtype_label}\n"
        f"shape: [{N}, {H}, {W}, {C}]\n"
        f"neg_slope: {neg_slope}\n"
        f"seed: {seed}\n"
    ...
    print(f"LeakyReLU FWD+BWD {dtype_label} reference generated in: {ws}")

def main():
    parser = argparse.ArgumentParser(description='LeakyReLU FWD+BWD test data generator')
