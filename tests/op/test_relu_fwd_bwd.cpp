/**
 * @file test_relu_fwd_bwd.cpp
 * @brief ReLU FWD+BWD 串接测试 — 支持 CPU / GPU / AMP 三种模式
 * @version 1.1.0
 * @date 2026-05-17
 * @author 技术觉醒团队
 *
 * 用法：
 *   test_relu_fwd_bwd.exe --cpu  [--shape 8,1024,1024,8]
 *   test_relu_fwd_bwd.exe --gpu  [--shape 8,1024,1024,8]
 *   test_relu_fwd_bwd.exe --amp  [--shape 8,1024,1024,8]
 *
 * 注意：
...
int main(int argc, char** argv) {
    ...
    std::string ws = std::string(TR_WORKSPACE) + "/relu_fwd_bwd_data";
    ...
    py << "python ";
#endif
    py << std::string(TR_PROJECT_ROOT) << "/tests/op/test_relu_fwd_bwd.py"
       << " --shape " << cfg.shape_str
       << " --seed " << cfg.seed
       << " --workspace \"" << ws << "\""
       << " --dtype " << py_dtype;
    ...
    ComputationGraph g;
    if (is_amp) {
        g.append(ComputeOp::RELU_AMP_FWD,  {d_x.id}, {d_y.id, d_mask.id});
        g.append(ComputeOp::RELU_AMP_BWD,  {d_dy.id, d_mask.id}, {d_dx.id});
    } else {
        g.append(ComputeOp::RELU_FP32_FWD, {d_x.id}, {d_y.id, d_mask.id});
        g.append(ComputeOp::RELU_FP32_BWD, {d_dy.id, d_mask.id}, {d_dx.id});
    }
    ...
    std::cout << "\n===== ReLU FWD+BWD " << mode_name(cfg.mode)
              << " (" << num_ranks << " rank(s)): "
              << (all_pass ? "PASS" : "FAIL") << " =====\n"
