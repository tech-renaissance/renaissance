 ...
 *   test_avgpool_fwd_bwd.exe --gpu  [--shape 8,16,16,64 --kernel 2 --stride 2 --padding 0]
 *   test_avgpool_fwd_bwd.exe --amp  [--shape 8,16,16,64 --kernel 2 --stride 2 --padding 0]
 *
 * 注意：
 *   FWD 与 BWD 分两次 run() 执行，BWD 采用 dX 覆盖 X 的 in-place 语义。
 *   AvgPool BWD 不需要 X 作为输入（仅 dY）。
 */
...
struct TestConfig {
    TestMode mode;
    std::string shape_str = "8,16,16,64";
    int kernel = 2;
    int stride = 2;
    int padding = 0;
    int seed = 42;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--cpu") {
            ...
            std::cout << "Usage: " << argv[0] << " --cpu|--gpu|--amp [options]\n\n"
                ...
                << "Options:\n"
                << "  --shape N,H,W,C    Input tensor shape (default: 8,16,16,64)\n"
                << "  --kernel N         Kernel size (default: 2)\n"
                << "  --stride N         Stride (default: 2)\n"
                << "  --padding N        Padding (default: 0)\n"
...
int main(int argc, char** argv) {
    ...
    std::string ws = std::string(TR_WORKSPACE) + "/avgpool_fwd_bwd_data";
    ...
    py << "python ";
#endif
    py << std::string(TR_PROJECT_ROOT) << "/tests/op/test_avgpool_fwd_bwd.py"
       << " --shape " << cfg.shape_str
       << " --kernel " << cfg.kernel
       << " --stride " << cfg.stride
       << " --padding " << cfg.padding
    ...
    auto& pool_params = yaml["pool_params"];
    int kernel = pool_params["kernel"].get_value<int>();
    int stride = pool_params["stride"].get_value<int>();
    int padding = pool_params["padding"].get_value<int>();

    // 计算输出形状
    int OH = (shape.h() + 2 * padding - kernel) / stride + 1;
    int OW = (shape.w() + 2 * padding - kernel) / stride + 1;
    Shape out_shape{shape.n(), OH, OW, shape.c()};
    ...
    PoolParams pp{kernel, kernel, stride, stride, padding, padding};

    // FWD graph: 仅输出 y
    ComputationGraph g_fwd;
    if (is_amp) {
        g_fwd.append(ComputeOp::AVGPOOL_AMP_FWD, {d_x.id}, {d_y.id}, OpParams(pp));
    } else {
        g_fwd.append(ComputeOp::AVGPOOL_FP32_FWD, {d_x.id}, {d_y.id}, OpParams(pp));
    }
    task.add_graph("fwd", std::move(g_fwd), StreamKind::COMP_2);

    // BWD graph: dX 覆盖 X (in-place)，AvgPool BWD 只需要 dy
    ComputationGraph g_bwd;
    if (is_amp) {
        g_bwd.append(ComputeOp::AVGPOOL_AMP_BWD,
                     {d_dy.id},
                     {d_x.id}, OpParams(pp));
    } else {
        g_bwd.append(ComputeOp::AVGPOOL_FP32_BWD,
                     {d_dy.id},
                     {d_x.id}, OpParams(pp));
    }
    ...
    std::cout << "\n===== AvgPool FWD+BWD " << mode_name(cfg.mode)
              << " (" << num_ranks << " rank(s)): "
              << (all_pass ? "PASS" : "FAIL") << " =====\n"
