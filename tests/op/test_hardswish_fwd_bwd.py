#!/usr/bin/env python3
"""
HardSwish FWD+BWD 串接测试 — PyTorch 参考数据生成
FWD:  y = x <= -3 ? 0 : x >= 3 ? x : x * (x + 3) / 6
BWD:  dx = x <= -3 ? 0 : x >= 3 ? dy : dy * (2*x + 3) / 6
支持 --dtype fp16 | fp32
"""
...
def generate(shape: tuple, seed: int, ws: str, dtype: str):
    ...
    Y = torch.where(X <= -3.0, torch.zeros_like(X),
           torch.where(X >= 3.0, X, X * (X + 3.0) / 6.0))

    # ── BWD ──
    torch.manual_seed(seed + 1000)
    dY = torch.randn(N, H, W, C, dtype=torch_dtype) * 2.0
    dX = torch.where(X <= -3.0, torch.zeros_like(dY),
            torch.where(X >= 3.0, dY, dY * (2.0 * X + 3.0) / 6.0))
    ...
    print(f"HardSwish FWD+BWD {dtype_label} reference generated in: {ws}")

def main():
    parser = argparse.ArgumentParser(description='HardSwish FWD+BWD test data generator')
