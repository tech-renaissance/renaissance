 ...
 *   test_maxpool_fwd_bwd.exe --gpu  [--shape 8,224,224,64 --kernel 3 --stride 2 --padding 1]
 *   test_maxpool_fwd_bwd.exe --amp  [--shape 8,224,224,64 --kernel 3 --stride 2 --padding 1]
 *
 * 注意：
 *   FWD 与 BWD 分两次 run() 执行，BWD 采用 dX 覆盖 X 的 in-place 语义。
 *   mask 不做数学验证（实现可能与 PyTorch 不同）。
 */
...
int main(int argc, char** argv) {
    ...
    std::string ws = std::string(TR_WORKSPACE) + "/maxpool_fwd_bwd_data";
    ...
    py << "python ";
#endif
    py << std::string(TR_PROJECT_ROOT) << "/tests/op/test_maxpool_fwd_bwd.py"
       << " --shape " << cfg.shape_str
       << " --kernel " << cfg.kernel
       << " --stride " << cfg.stride
       << " --padding " << cfg.padding
    ...
    if (is_amp) {
        g_fwd.append(ComputeOp::MAXPOOL_AMP_FWD, {d_x.id}, {d_y.id, d_mask.id}, OpParams(pp));
    } else {
        g_fwd.append(ComputeOp::MAXPOOL_FP32_FWD, {d_x.id}, {d_y.id, d_mask.id}, OpParams(pp));
    }
    task.add_graph("fwd", std::move(g_fwd), StreamKind::COMP_2);

    // BWD graph: dX 覆盖 X (in-place)，接口统一为 4 输入 {dy, y, mask, x}
    ComputationGraph g_bwd;
    if (is_amp) {
        g_bwd.append(ComputeOp::MAXPOOL_AMP_BWD,
                     {d_dy.id, d_y.id, d_mask.id, d_x.id},
                     {d_x.id}, OpParams(pp));
    } else {
        g_bwd.append(ComputeOp::MAXPOOL_FP32_BWD,
                     {d_dy.id, d_y.id, d_mask.id, d_x.id},
                     {d_x.id}, OpParams(pp));
    }
    ...
    for (int rank = 0; rank < num_ranks; ++rank) {
        ...
        std::cout << "  Rank " << rank << " FWD MSE = " << std::scientific
                  << mse_y;
        if (mse_y > mse_thr_fwd) { std::cout << "  FAIL"; all_pass = false; }
        std::cout << std::endl;
    }
    ...
    for (int rank = 0; rank < num_ranks; ++rank) {
        ...
        std::cout << "  Rank " << rank << " BWD MSE = " << std::scientific
                  << mse_dx;
        if (mse_dx > mse_thr_bwd) { std::cout << "  FAIL"; all_pass = false; }
        std::cout << std::endl;
    }

    std::cout << "\n===== MaxPool FWD+BWD " << mode_name(cfg.mode)
              << " (" << num_ranks << " rank(s)): "
              << (all_pass ? "PASS" : "FAIL") << " =====\n"
