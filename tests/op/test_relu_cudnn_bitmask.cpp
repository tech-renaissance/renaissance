...
class ReLUCuDNNBitmaskBenchmark {
private:
    ...
    std::shared_ptr<fe::graph::Tensor_attributes> bwd_mask_;
...
private:
    ...
    void build_relu_bwd_graph() {
        ...
        // 对齐 *_old.cpp: BWD 取 FWD 的 bitmask，用 MUL 而非 RELU_BWD
        bwd_mask_ = relu_bwd_graph_->tensor(fe::graph::Tensor_attributes()
            .set_name("mask")
            .set_dim(dim)
            .set_stride(stride)
        ...
        auto dx_out = relu_bwd_graph_->pointwise(bwd_dy_, bwd_mask_, mul_options);
        bwd_dx_ = dx_out;
...
public:
    void run() {
        ...
        std::cout << "===== ReLU cuDNN Bitmask FP16 (1 GPU) =====\n"
                  << "  Shape: " << N << "x" << H << "x" << W << "x" << C
                  << " (" << mb << " MB FP16)\n"
...
int main() {
    ReLUCuDNNBitmaskBenchmark bench;
    bench.run();
    return 0;
}