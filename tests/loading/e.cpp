// EXPERT_OP - Direct I/O优化方案
// 使用O_DIRECT/FILE_FLAG_NO_BUFFERING，NUMA感知分配
...
void worker_thread(ThreadContext* ctx, const std::vector<std::vector<FileInfo>>& folder_files) {
    ...
    for (size_t folder_idx = ctx->start_folder; folder_idx < ctx->end_folder; ++folder_idx) {
        if (folder_idx >= folder_files.size()) break;

        const auto& files = folder_files[folder_idx];
...
int main(int argc, char* argv[]) {
    ...
    if (base_path.empty()) {
        ...
        std::cerr << "Usage: " << argv[0] << " --path <directory> --workers <1|2|4|8|16>" << std::endl;
        return 1;
    }
    ...
    for (size_t i = 0; i < folders.size(); ++i) {
        for (const auto& entry : fs::directory_iterator(folders[i])) {
            if (entry.is_regular_file()) {
                folder_files[i].push_back({entry.path().string(), entry.file_size()});
            }
        }
    }
    ...
    for (int i = 0; i < num_workers; ++i) {
        ...
        contexts[i].start_folder = i * (folders.size() / num_workers);
        contexts[i].end_folder = (i + 1) * (folders.size() / num_workers);
        if (i == num_workers - 1) contexts[i].end_folder = folders.size();
    ...
    for (int i = 0; i < num_workers; ++i) {
        threads.emplace_back(worker_thread, &contexts[i], std::cref(folder_files));
    }
