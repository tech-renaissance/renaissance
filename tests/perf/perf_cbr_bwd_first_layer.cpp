/**
 * @file perf_cbr_bwd_first_layer.cpp
 * @brief CBR_AMP_BWD_FIRST_LAYER 性能测试
 *
 * 默认形状: 输入 [512,224,224,4], 卷积核 7x7, stride=2, pad=3, 输出 112x112x64
 * 先跑一次 CBR_AMP_FWD 填充中间结果, 再计时 CBR_AMP_BWD_FIRST_LAYER 100 次取平均
 */
...
int main() {
    ...
    std::cout << "===== CBR_AMP_BWD_FIRST_LAYER 性能测试 =====" << std::endl;
    ...
    // CBR_AMP_FWD inputs (首层权重区域)
    DTensor d_x        = task.alloc(x_shape, DType::FP16, Region::F_FEATURE_FP16);
    DTensor d_w        = task.alloc(w_shape, DType::FP16, Region::A_FIRST_CONV);
    ...
    // CBR_AMP_BWD_FIRST_LAYER inputs / outputs (dx 复用 d_x 做 in-place)
    DTensor d_dy     = task.alloc(y_shape, DType::FP16, Region::F_FEATURE_FP16);
    DTensor d_gw     = task.alloc(w_shape, DType::FP16, Region::G_FIRST_CONV_FP16);
    ...
    // BWD_FIRST_LAYER graph: dx 输出复用 d_x (in-place), 权重梯度区域为 G_FIRST_CONV_FP16
    ComputationGraph g_bwd;
    g_bwd.append(ComputeOp::CBR_AMP_BWD_FIRST_LAYER,
        {d_dy.id, d_w.id, d_bn_w.id,
         d_saved_mean.id, d_saved_inv_var.id, d_relu_mask.id, d_x.id},
        {d_x.id, d_gw.id, d_gamma.id, d_beta.id,
    ...
    std::cout << "===== CBR_AMP_BWD_FIRST_LAYER 性能 =====" << std::endl;
