/**
 * @file test_maxpool_perf.cpp
 * @brief MaxPool 性能测试 —— FWD/BWD/INF 单独计时
 * @version 1.0.0
 * @date 2026-06-03
 *
 * 固定配置：
 *   - 输入形状: [512, 112, 112, 64] (NHWC)
 *   - Pool参数: kernel=3, stride=2, padding=1
 *   - 输出形状: [512, 56, 56, 64]
 *   - 模式: 支持 --cpu / --gpu / --amp
 *   - 预热：5次
 *   - 计时：100次取平均
 *
 * 用法：
 *   test_maxpool_perf.exe --cpu
 *   test_maxpool_perf.exe --gpu
 *   test_maxpool_perf.exe --amp
 */
...
int main(int argc, char** argv) {
    // 固定配置
    const int N = 512, H = 112, W = 112, C = 64;
    const int K = 3, S = 2, P = 1;
    int OH = (H + 2 * P - K) / S + 1;  // 56
    int OW = (W + 2 * P - K) / S + 1;  // 56
    const int warmup = 5;
    ...
    std::cout << "===== MaxPool 性能测试 =====" << std::endl;
    ...
    if (is_amp) {
        ...
        Tensor h_dy = Tensor::randn_fp16(out_shape, DType::FP16, 0.0f, 0.1f);
        task.transfer_to_rank(h_x, d_x, 0);
        task.transfer_to_rank(h_dy, d_dy, 0);
    } else {
        Tensor h_x  = Tensor::randn(in_shape,  DType::FP32, 0.0f, 0.1f);
        Tensor h_dy = Tensor::randn(out_shape, DType::FP32, 0.0f, 0.1f);
        task.transfer_to_rank(h_x, d_x, 0);
        task.transfer_to_rank(h_dy, d_dy, 0);
    }

    // ===== FWD 性能测试 =====
    std::cout << "===== MaxPool FWD 性能测试 =====" << std::endl;
