/**
 * @file test_musa_device_rng.cpp
 * @brief MusaDevice高级RNG接口测试
...
void test_uniform() {
    ...
    // 测试uniform方法
    Tensor t = musa.uniform(Shape({1000}), 0.0f, 10.0f);

    // 同步到Host进行检查
    std::vector<float> host_data(1000);
    musaError_t err = musaMemcpy(host_data.data(), t.data_ptr(),
                                  1000 * sizeof(float), musaMemcpyDeviceToHost);
    if (err != musaSuccess) {
        std::cerr << "  musaMemcpy failed: " << musaGetErrorString(err) << std::endl;
...
void test_uniform_inplace() {
    ...
    // 创建张量并原地填充随机数
    Tensor t = musa.zeros(Shape({1000}), DType::FP32);
    musa.uniform_inplace(t, 0.0f, 10.0f);
...
void test_randn() {
    ...
    Tensor t = musa.randn(Shape({100000}), 0.0f, 1.0f);

    // 同步到Host
    std::vector<float> host_data(100000);
    musaError_t err = musaMemcpy(host_data.data(), t.data_ptr(),
                                  100000 * sizeof(float), musaMemcpyDeviceToHost);
    if (err != musaSuccess) {
        std::cerr << "  musaMemcpy failed: " << musaGetErrorString(err) << std::endl;
        std::cout << "  FAIL" << std::endl;
        return;
...
void test_randn_inplace() {
    ...
    // 创建张量并原地填充随机数
    Tensor t = musa.zeros(Shape({100000}), DType::FP32);
    musa.randn_inplace(t, 0.0f, 1.0f);
...
void test_randint_fp32() {
    ...
    Tensor t = musa.randint(Shape({10000}), 0, 10, DType::FP32);

    // 同步到Host
    std::vector<float> host_data(10000);
    musaError_t err = musaMemcpy(host_data.data(), t.data_ptr(),
                                  10000 * sizeof(float), musaMemcpyDeviceToHost);
    if (err != musaSuccess) {
        std::cerr << "  musaMemcpy failed: " << musaGetErrorString(err) << std::endl;
        std::cout << "  FAIL" << std::endl;
        return;
...
void test_randint_int32() {
    ...
    Tensor t = musa.randint(Shape({10000}), -5, 5, DType::INT32);

    // 同步到Host
    std::vector<int32_t> host_data(10000);
    musaError_t err = musaMemcpy(host_data.data(), t.data_ptr(),
                                  10000 * sizeof(int32_t), musaMemcpyDeviceToHost);
    if (err != musaSuccess) {
        std::cerr << "  musaMemcpy failed: " << musaGetErrorString(err) << std::endl;
        std::cout << "  FAIL" << std::endl;
        return;
...
void test_randint_inplace() {
    ...
    // 创建张量并原地填充随机数
    Tensor t = musa.zeros(Shape({10000}), DType::INT32);
    musa.randint_inplace(t, 0, 100, DType::INT32);
...
void test_randbool_fp32() {
    ...
    Tensor t = musa.randbool(Shape({10000}), 0.5f, DType::FP32);
...
void test_randbool_int32() {
    ...
    Tensor t = musa.randbool(Shape({10000}), 0.3f, DType::INT32);
...
void test_randbool_inplace() {
    ...
    // 创建张量并原地填充随机数
    Tensor t = musa.zeros(Shape({10000}), DType::FP32);
    musa.randbool_inplace(t, 0.5f, DType::FP32);
...
void test_dtype_errors() {
    ...
    try {
        Tensor t = musa.uniform(Shape({100}), 0.0f, 1.0f, DType::INT32);
    ...
    try {
        Tensor t = musa.randn(Shape({100}), 0.0f, 1.0f, DType::INT32);
    } catch (const TypeError&) {
        randn_error = true;
    ...
    try {
        Tensor t = musa.randint(Shape({100}), 0, 10, DType::INT8);
    } catch (const TypeError&) {
    ...
    try {
        Tensor t = musa.randbool(Shape({100}), 0.5f, DType::BF16);
    } catch (const TypeError&) {
...
int main() {
    std::cout << "========================================" << std::endl;
    std::cout << "  MusaDevice RNG High-Level API Test" << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    try {
        ...
        std::cout << "\n========================================" << std::endl;
        std::cout << "  All MusaDevice RNG tests passed!" << std::endl;
        std::cout << "========================================" << std::endl;
