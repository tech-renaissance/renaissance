/**
 * @file test_sync_vs_async_perf_musa.cpp
 * @brief 同步传输 vs 异步传输性能对比测试（MUSA版本）
 * @version 3.6.18
 * @date 2026-01-03
 * @author 技术觉醒团队
 */
...
int main() {
    ...
    try {
        ...
        Tensor tensor_sync = musa.zeros(shape, DType::FP32);
        ...
        // 分配锁页内存
        auto pinned = musa.alloc_pinned(data_size);
        ...
        Tensor tensor_async = musa.zeros(shape, DType::FP32);

        auto start_async = std::chrono::high_resolution_clock::now();
        musa.async_copy_h2d(host_data, tensor_async);
