...
int main() {
    std::cout << "========================================" << std::endl;
    std::cout << "  CPU Cast Performance Test (V3.6.18)" << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    try {
        ...
        Tensor fp32_tensor = cpu.randn(shape, 0.0f, 1.0f, DType::FP32);
        std::cout << "  Created: " << fp32_tensor.nbytes() / (1024.0 * 1024.0)
                  << " MB" << std::endl;

        // 2. 创建BF16张量
        std::cout << "\n[2/9] Creating BF16 tensor (zeros)..." << std::endl;
        Tensor bf16_tensor = cpu.zeros(shape, DType::BF16);
        ...
        timer.reset();
        cpu.cast_into(bf16_tensor, fp32_tensor);
        ...
        Tensor int32_tensor = cpu.randint(shape, -1000000, 1000000, DType::INT32);
        std::cout << "  Created: " << int32_tensor.nbytes() / (1024.0 * 1024.0)
                  << " MB" << std::endl;

        // 6. INT32 -> FP32
        std::cout << "\n[6/9] Testing INT32 -> FP32..." << std::endl;
        timer.reset();
        cpu.cast_into(int32_tensor, fp32_tensor);
        ...
        timer.reset();
        cpu.cast_into(fp32_tensor, int32_tensor);
        ...
        Tensor int8_tensor = cpu.zeros(shape, DType::INT8);
        std::cout << "  Created: " << int8_tensor.nbytes() / (1024.0 * 1024.0)
                  << " MB" << std::endl;

        // 9. INT32 -> INT8
        std::cout << "\n[9/9] Testing INT32 -> INT8..." << std::endl;
        timer.reset();
        cpu.cast_into(int32_tensor, int8_tensor);
        ...
        timer.reset();
        cpu.cast_into(int8_tensor, int32_tensor);
