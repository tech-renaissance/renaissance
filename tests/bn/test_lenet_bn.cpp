/**
 * @file test_lenet_bn.cpp
 * @brief LeNet-5 with BatchNorm 准确率测试
 * @version 1.0.0
 * @date 2026-05-30
 * @author Team Tech-Renaissance
 *
 * 核心策略：
 * - 网络结构：channel_padding → conv(8,5,1,2) → bn → avgpool(2,2,0)
 *             → conv(16,5,1,0) → bn → avgpool(2,2,0)
 *             → fc(120) → bn → fc(88) → bn → fc(10)
...
int main(int argc, char** argv) {
    ...
    std::cout << "\n=====================================\n"
              ...
              << "=====================================\n"
              << "Network: channel_padding → conv(8,5,1,2) → bn → avgpool(2)"
              << " → conv(16,5,1,0) → bn → avgpool(2) → fc(120) → bn → fc(88) → bn → fc(10)\n"
