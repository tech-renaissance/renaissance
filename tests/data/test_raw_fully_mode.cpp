/**
 * @file test_raw_fully_mode.cpp
 * @brief RAW Loader FULLY模式完整性和速度测试
 * @details 测试ImageNetLoaderRaw的FULLY模式，验证数据完整性和加载速度
 * @version 1.0.0
 * @date 2026-01-31
 * @author 技术觉醒团队
 */
...
void print_usage(const char* program_name) {
    std::cout << "Usage: " << program_name << " [OPTIONS]\n\n"
              ...
              << "Cache Performance:\n"
              << "  By default, this test measures WARM CACHE performance (fast).\n"
              << "  Use --clear-cache to measure COLD CACHE performance (real I/O speed).\n"
              << "  For cold cache test, you may need admin rights on Windows.\n\n"
              << "Examples:\n"
              << "  " << program_name << " --path /data/imagenet --val\n"
              << "  " << program_name << " --path /data/imagenet --val --workers 16\n"
              << "  " << program_name << " --path /data/imagenet --val --log\n"
              << "  " << program_name << " --path /data/imagenet --val --clear-cache  # Cold cache test\n";
}
...
void print_results(double time_sec, size_t total_samples,
    ...
    std::cout << "\n========================================\n"
              << "RAW FULLY Mode Test Results: " << dataset_name << "\n"
              << "========================================\n"
...
int main(int argc, char** argv) {
    ...
    std::cout << "========================================\n"
              << "RAW FULLY Mode Test: " << (is_train ? "Training" : "Validation") << " Set\n"
              << "========================================\n"
    ...
    try {
        ...
        loader.configure(
            num_workers,
            num_preprocess,
            dataset_path,   // train_path（用于训练集和验证集）
            dataset_path,   // val_path（同一路径，内部区分train/val子文件夹）
        ...
        // 开始epoch（包含首次加载和初始化洗牌）
        std::cout << "\n[3/5] Beginning epoch 0..." << std::endl;
        std::cout << "NOTE: This test measures FIRST EPOCH performance (load + shuffle)." << std::endl;
        std::cout << "      Subsequent epochs will be much faster (~0.05s, shuffle only)." << std::endl;
        ...
        std::cout << "\nStandard exception caught:" << std::endl;
