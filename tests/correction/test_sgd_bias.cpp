/**
 * @file test_sgd_bias.cpp
 * @brief RANGE_UPDATE_BIAS_SGD 数学正确性测试
 * @version 4.20.1
 * @date 2026-06-28
 * @author 技术觉醒团队
 ...
 *
 * 设计意图：
 *   验证 SGD（无 momentum）Bias 更新算子的数值正确性。
 *   更新公式：w = w - lr * g  （Bias 无 weight decay）
 */
...
int main(int argc, char* argv[]) {
    ...
    Shape shape_a{1, 1, 1, 64};
    Shape shape_b{1, 1, 1, 128};
    ...
    ComputationGraph g;
    {
        ...
        node.range_op = RangeOp::RANGE_UPDATE_BIAS_SGD;
        node.input_ranges = {
    ...
    task.add_graph("sgd_bias", std::move(g), StreamKind::UPDATE);
    task.compile();
    ...
    Tensor h_w_a = Tensor::fill(shape_a, DType::FP32, 0.75f);
    Tensor h_w_b = Tensor::fill(shape_b, DType::FP32, -0.35f);
    Tensor h_g_a = Tensor::fill(shape_a, DType::FP32, 0.2f);
    Tensor h_g_b = Tensor::fill(shape_b, DType::FP32, -0.15f);
    ...
    auto expected = [&](const Tensor& w, const Tensor& g) -> Tensor {
        Tensor e(w.shape(), DType::FP32);
        for (int64_t i = 0; i < w.numel(); ++i) {
            e.data<float>()[i] = w.data<float>()[i] - lr_val * g.data<float>()[i];
        }
        return e;
    };
    ...
    for (int rank = 0; rank < num_ranks; ++rank) {
        ...
        Tensor h_exp_a = expected(h_w_a, h_g_a);
        Tensor h_exp_b = expected(h_w_b, h_g_b);
    ...
    std::cout << "\nRANGE_UPDATE_BIAS_SGD: " << (all_pass ? "PASS" : "FAIL") << std::endl;
    return all_pass ? 0 : 1;
}