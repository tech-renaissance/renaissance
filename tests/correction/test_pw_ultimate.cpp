...
// Windows宏冲突修复：Windows头文件定义了min/max宏，影响std::min和std::max
#ifdef _WIN32
    #ifdef min
        #undef min
    #endif
...
void print_usage(const char* program_name) {
    std::cout << "Usage: " << program_name << " [OPTIONS]\n\n"
              ...
              << "  --normalization <M>  Normalization mode for train and val (default: mlperf):\n"
              << "                      no_norm, mnist, cifar, imagenet, mlperf\n"
              << "                      If not specified, defaults to MLPERF (divide-by-255 normalization)\n"
              ...
              << "  --gpu-ids <IDS>      GPU IDs (e.g., \"0,1,2,3\" or \"0-7\")\n"
              << "                      Default: auto-detect all visible GPUs (only for GPU mode)\n"
              ...
              << "PO Parameters (Normalization):\n"
              << "  --normalization <M>  Available modes:\n"
              << "                      no_norm  - Identity normalization (divide by 1, subtract 0)\n"
              << "                      mnist    - MNIST: mean=0.1307, std=0.3081\n"
              << "                      cifar    - CIFAR-10: mean=(0.4914,0.4822,0.4465), std=(0.2470,0.2435,0.2616)\n"
              << "                      imagenet - ImageNet: mean=(0.485,0.456,0.406), std=(0.229,0.224,0.225)\n"
              << "                      mlperf   - MLPerf: mean=(123.68,116.78,103.94)/255, std=1/255\n\n"
              << "Examples:\n"
              << "  " << program_name << " --dataset imagenet --path /data/imagenet --po-train1 RandomResizedCrop --po-train2 RandomHorizontalFlip --normalization imagenet\n"
              << "  " << program_name << " --dataset imagenet --path /data/imagenet --po-train1 FastRandomResizedCrop --sdmp 2 --epoch 2 --normalization imagenet\n"
              << "  " << program_name << " --dataset imagenet --path /data/imagenet --po-train1 RandomResizedCrop --po-train2 GaussianBlur --resolution 256\n"
              << "  " << program_name << " --dataset mnist --path /data/mnist --format raw --mode fully --po-train1 Resize --normalization mnist\n"
              << "  " << program_name << " --dataset cifar10 --path /data/cifar --po-train1 CenterCrop --sdmp 3 --epoch 2 --normalization cifar\n"
              << "  " << program_name << " --dataset imagenet --path /data/imagenet --po-train1 RandomResizedCrop --reproducible\n\n"
              << "Note: This test uses Preprocessor NORMAL mode (with TransferStation)\n\n"
              ...
              << "Command Sample: /root/epfs/R/renaissance/build/bin/tests/pw/test_pw_ultimate --dataset imagenet --path /root/epfs/dataset/imagenet --format raw --lv 0 --mode fully --cpu-bind --batch-size 512 --resolution 224 --loaders 16 --preproc 112 --device GPU --gpu-ids \"0,1,2,3,4,5,6,7\" --epoch 28 --po-train1 FastRandomResizedCrop --po-train2 RandomHorizontalFlip --po-val1 Resize --po-val2 CenterCrop --normalization imagenet --seed 42 --sdmp 2 --cpvs true\n\n";
}
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "AMP: " << (using_amp ? "enabled" : "disabled") << "\n";
    std::cout << "Test mode: false (NORMAL mode with TransferStation)\n";
    ...
    Preprocessor::setup()
        ...
        .normalization(parse_norm_mode(normalization_mode))
