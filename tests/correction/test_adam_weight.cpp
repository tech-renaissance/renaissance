/**
 * @file test_adam_weight.cpp
 * @brief RANGE_UPDATE_WEIGHT_ADAM 数学正确性测试
 * @version 4.20.1
 * @date 2026-06-28
 * @author 技术觉醒团队
 ...
 *
 * 设计意图：
 *   验证 Adam Weight 更新算子的数值正确性。
 *   更新公式：m = m * b1 + (1 - b1) * g
 *            v = v * b2 + (1 - b2) * g^2
 *            w = w * (1 - lr * wd) - lr * m / (sqrt(v) + eps)
 */
...
int main(int argc, char* argv[]) {
    ...
    Tensor h_w_b = Tensor::fill(shape_b, DType::FP32, -0.5f);
    Tensor h_g_a = Tensor::fill(shape_a, DType::FP32, 0.08f);
    Tensor h_g_b = Tensor::fill(shape_b, DType::FP32, -0.06f);
    Tensor h_m_a = Tensor::fill(shape_a, DType::FP32, 0.01f);
    Tensor h_m_b = Tensor::fill(shape_b, DType::FP32, -0.01f);
    Tensor h_v_a = Tensor::fill(shape_a, DType::FP32, 0.02f);
    Tensor h_v_b = Tensor::fill(shape_b, DType::FP32, 0.015f);
    ...
    std::cout << "\nRANGE_UPDATE_WEIGHT_ADAM: " << (all_pass ? "PASS" : "FAIL") << std::endl;
    return all_pass ? 0 : 1;
}
