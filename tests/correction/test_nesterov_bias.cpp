/**
 * @file test_nesterov_bias.cpp
 * @brief RANGE_UPDATE_BIAS_NESTEROV 数学正确性测试
 * @version 4.20.1
 * @date 2026-06-28
 * @author 技术觉醒团队
 ...
 *
 * 设计意图：
 *   验证 Nesterov Bias 更新算子的数值正确性。
 *   更新公式：m_new = m * beta + g;  m = m_new;
 *            w = w - lr * (m_new * beta + g)  （Bias 无 weight decay）
 */
...
int main(int argc, char* argv[]) {
    ...
    Shape shape_a{1, 1, 1, 112};
    Shape shape_b{1, 1, 1, 176};
    ...
    ComputationGraph g;
    {
        ...
        node.range_op = RangeOp::RANGE_UPDATE_BIAS_NESTEROV;
        node.input_ranges = {
    ...
    task.add_graph("nesterov_bias", std::move(g), StreamKind::UPDATE);
    task.compile();

    float lr_val       = 0.02f;
    float beta_val     = 0.85f;
    ...
    Tensor h_w_a = Tensor::fill(shape_a, DType::FP32, 0.33f);
    Tensor h_w_b = Tensor::fill(shape_b, DType::FP32, -0.27f);
    Tensor h_g_a = Tensor::fill(shape_a, DType::FP32, -0.11f);
    Tensor h_g_b = Tensor::fill(shape_b, DType::FP32, 0.14f);
    ...
    std::cout << "\nRANGE_UPDATE_BIAS_NESTEROV: " << (all_pass ? "PASS" : "FAIL") << std::endl;
    return all_pass ? 0 : 1;
}