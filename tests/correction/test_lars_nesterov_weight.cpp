/**
 * @file test_lars_nesterov_weight.cpp
 * @brief LARS_COMPUTE_TRUST_RATIO + LARS_NESTEROV_UPDATE 数学正确性测试
 * @version 1.0.0
 * @date 2026-05-21
 * @author 技术觉醒团队
 *
 * 设计意图：
 *   验证 LARS NESTEROV Weight 更新算子的数值正确性。
 *   更新公式：
 ...
 *     W = W - lr·(η·G' + β·M_new)
 *
 *   与标准LARS的唯一差异在Weight更新公式（Nesterov前瞻项）
 */
...
int main(int argc, char* argv[]) {
    ...
    // 构造ComputationGraph（4个COMPUTE节点：2个trust_ratio + 2个nesterov_update）
    ComputationGraph g;
    ...
    // DTensor A: Step 2 = LARS NESTEROV Update
    {
        GraphNode node;
        node.kind = GraphNode::Kind::COMPUTE;
    ...
    // DTensor B: Step 2 = LARS NESTEROV Update
    {
        GraphNode node;
        node.kind = GraphNode::Kind::COMPUTE;
        node.compute_op = ComputeOp::LARS_NESTEROV_UPDATE;
    ...
    task.add_graph("lars_nesterov_weight", std::move(g), StreamKind::UPDATE);
    task.compile();
    ...
    auto expected_lars_nesterov = [&](const Tensor& w, const Tensor& g,
    ...
    for (int rank = 0; rank < num_ranks; ++rank) {
        ...
        auto [h_exp_w_a, h_exp_m_a] = expected_lars_nesterov(h_w_a, h_g_a, h_m_a, eta_a);
        auto [h_exp_w_b, h_exp_m_b] = expected_lars_nesterov(h_w_b, h_g_b, h_m_b, eta_b);
    ...
    std::cout << "\nLARS_COMPUTE_TRUST_RATIO + LARS_NESTEROV_UPDATE: "
