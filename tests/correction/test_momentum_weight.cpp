/**
 * @file test_momentum_weight.cpp
 * @brief RANGE_UPDATE_WEIGHT_MOMENTUM 数学正确性测试
 * @version 1.0.0
 * @date 2026-05-21
 * @author 技术觉醒团队
 *
 * 设计意图：
 *   验证 Momentum Weight 更新算子的数值正确性。
 *   更新公式：m = m * beta + g;  w = w * (1 - lr*wd) - lr * m
 */
...
int main(int argc, char* argv[]) {
    ...
    Shape shape_a{4, 8, 4, 4};
    Shape shape_b{8, 4, 8, 2};
    ...
    ComputationGraph g;
    {
        ...
        node.input_ids = {d_lr.id, d_wd.id, d_beta.id};
        g.append(std::move(node));
    }
    task.add_graph("momentum_weight", std::move(g), StreamKind::UPDATE);
    task.compile();
    ...
    Tensor h_w_b = Tensor::fill(shape_b, DType::FP32, -1.0f);
    Tensor h_g_a = Tensor::fill(shape_a, DType::FP32, 0.12f);
    Tensor h_g_b = Tensor::fill(shape_b, DType::FP32, -0.08f);
    Tensor h_m_a = Tensor::fill(shape_a, DType::FP32, 0.05f);
    Tensor h_m_b = Tensor::fill(shape_b, DType::FP32, -0.03f);
    ...
    auto expected = [&](const Tensor& w, const Tensor& g,
        ...
        for (int64_t i = 0; i < w.numel(); ++i) {
            float m_i = m_in.data<float>()[i] * beta_val + g.data<float>()[i];
            e_w.data<float>()[i] = w.data<float>()[i] * decay - lr_val * m_i;
        }
        return e_w;
    };
    ...
    std::cout << "\nRANGE_UPDATE_WEIGHT_MOMENTUM: " << (all_pass ? "PASS" : "FAIL") << std::endl;
    return all_pass ? 0 : 1;
}