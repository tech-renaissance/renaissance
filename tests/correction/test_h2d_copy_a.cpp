 ...
 * @brief RANGE_H2D_COPY_A 数学正确性测试（FP32 + AMP）
 * @version 4.20.1
 * @date 2026-06-28
 * @author 技术觉醒团队
 ...
 *
 * 设计意图：
 *   验证 RANGE_H2D_COPY_A 异步 H2D 传输的数据正确性。
 *   通过 SimpleTask 构造包含 RANGE_H2D_COPY_A 节点的 TRANSFER graph，
 *   compile_capture_simple 自动分配 StagingBufferPool。
 *   将已知数据填充到 StagingBufferPool A 区，执行异步传输后
 *   通过 fetch_from_rank 同步取回，对比 MSE 与 max|diff|。
 *
 *   默认 FP32 模式：INT32 label + FP32 data，channels=1
 *   --amp 模式：INT32 label + FP16 data，channels=3→4（AMP 自动填充）
 *
 *   使用方法：
 *     test_h2d_copy_a --gpu       (GPU FP32, 默认)
 *     test_h2d_copy_a --cpu       (CPU FP32)
 *     test_h2d_copy_a --amp       (GPU AMP)
 */
...
int main(int argc, char* argv[]) {
    ...
    // 即使本测试只使用A区，也必须分配B区张量以确保地址和容量计算正确
    ...
    DTensor d_b_data_unused  = task.alloc(Shape{batch_size, resolution, resolution, effective_c},
    ...
    size_t data_slot = static_cast<size_t>(DistributedTensor::compute_slot_bytes(
            ...
            data_dtype, Region::I_A_DATA));
    (void)data_slot;  // A区测试不需要data_slot，保留以备将来扩展
    ...
    if (use_amp) {
        ...
        for (int rank = 0; rank < num_ranks; ++rank) {
            ...
            std::memcpy(base, h_label.data<int32_t>(), static_cast<size_t>(batch_size) * sizeof(int32_t));
            std::memcpy(base + label_aligned, src_fp16.data(), data_bytes);
        }
        ...
        std::cout << "\nRANGE_H2D_COPY_A AMP (" << num_ranks << " ranks): "
