/**
 * @file test_momentum_bias.cpp
 * @brief RANGE_UPDATE_BIAS_MOMENTUM 数学正确性测试
 * @version 4.20.1
 * @date 2026-06-28
 * @author 技术觉醒团队
 ...
 *
 * 设计意图：
 *   验证 Momentum Bias 更新算子的数值正确性。
 *   更新公式：m = m * beta + g;  w = w - lr * m  （Bias 无 weight decay）
 */
...
int main(int argc, char* argv[]) {
    ...
    Shape shape_a{1, 1, 1, 96};
    Shape shape_b{1, 1, 1, 160};
    ...
    ComputationGraph g;
    {
        ...
        node.range_op = RangeOp::RANGE_UPDATE_BIAS_MOMENTUM;
        node.input_ranges = {
    ...
    Tensor h_w_b = Tensor::fill(shape_b, DType::FP32, -0.6f);
    Tensor h_g_a = Tensor::fill(shape_a, DType::FP32, 0.3f);
    Tensor h_g_b = Tensor::fill(shape_b, DType::FP32, 0.18f);
    Tensor h_m_a = Tensor::fill(shape_a, DType::FP32, 0.02f);
    Tensor h_m_b = Tensor::fill(shape_b, DType::FP32, -0.04f);
    ...
    auto expected = [&](const Tensor& w, const Tensor& g,
        ...
        for (int64_t i = 0; i < w.numel(); ++i) {
            ...
            e_w.data<float>()[i] = w.data<float>()[i] - lr_val * m_i;
        }
        return e_w;
    };
    ...
    std::cout << "\nRANGE_UPDATE_BIAS_MOMENTUM: " << (all_pass ? "PASS" : "FAIL") << std::endl;
    return all_pass ? 0 : 1;
}