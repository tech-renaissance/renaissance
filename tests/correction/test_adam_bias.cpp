/**
 * @file test_adam_bias.cpp
 * @brief RANGE_UPDATE_BIAS_ADAM 数学正确性测试
 * @version 4.20.1
 * @date 2026-06-28
 * @author 技术觉醒团队
 ...
 *
 * 设计意图：
 *   验证 Adam Bias 更新算子的数值正确性。
 *   更新公式：m = m * b1 + (1 - b1) * g
 *            v = v * b2 + (1 - b2) * g^2
 *            w = w - lr * m / (sqrt(v) + eps)  （Bias 无 weight decay）
 */
...
int main(int argc, char* argv[]) {
    ...
    Shape shape_a{1, 1, 1, 80};
    Shape shape_b{1, 1, 1, 144};
    ...
    ComputationGraph g;
    {
        ...
        node.range_op = RangeOp::RANGE_UPDATE_BIAS_ADAM;
        node.input_ranges = {
    ...
    task.add_graph("adam_bias", std::move(g), StreamKind::UPDATE);
    task.compile();
    ...
    Tensor h_w_b = Tensor::fill(shape_b, DType::FP32, 0.2f);
    ...
    std::cout << "\nRANGE_UPDATE_BIAS_ADAM: " << (all_pass ? "PASS" : "FAIL") << std::endl;
    return all_pass ? 0 : 1;
}
