/**
 * @file cbr_bwd_fp16_single.cpp
 * @brief cuDNN Frontend Conv+BN+ReLU反向融合性能测试V1.3 (FP16版本，单流执行：所有操作在同一流)
 * @version 1.3.0
 * @date 2026-05-16
 * @author 技术觉醒团队
 ...
 * @note 所属系列: cuda
 *
 * @note V1.3 改动（基于V1.2，简化为单流架构）：
 *   1. 【修改】反向拓扑：所有操作在S1单流上顺序执行
 *   2. 【删除】S2和S3流，只保留S1流
 *   3. 【删除】所有事件同步机制
 *   4. 【保留】所有其他功能和优化策略
 *   5. 【研究用途】对比单流vs多流的性能差异
 *
 * @note 单流反向拓扑结构：
 *       单流拓扑（所有操作在S1上顺序执行）：
 *         S1 (Begin) ──→ [BN Backward] ──→ [WGrad] ──→ [DGrad] ──→ S1 (End)
 *
 * @note 关键变化：
 *       - 移除了stream_comp_1_和stream_comp_1_，只使用stream_comp_1_
 *       - 移除了所有事件：event_comp_1_bwd_done_, event_bwd_join_s2_, event_bwd_join_s3_
 *       - 移除了handle_comp_1_和handle_comp_1_，只使用handle_comp_1_
 *       - 操作顺序：BN Backward → WGrad → DGrad（全部在S1流上）
 *       - 研究价值：量化多流架构的开销和收益
 */
...
private:
    // ========== 流和事件资源（单流版本）==========
    cudnnHandle_t handle_comp_1_;  // 只用S1流
    cudaStream_t stream_comp_1_;   // 单流执行所有操作
    ...
    // Workspace
    void *d_workspace_comp_1_;  // 单流workspace
...
public:
    ConvBNReLUBackwardBenchmark(const Config& config)
        ...
        std::cout << "Conv Backward Type: BN+DGrad+WGrad (SINGLE STREAM: All operations on S1)" << std::endl;
...
private:
    ...
    void build_conv_dgrad_graph() {
        ...
        CHECK_CUDNN_FE(conv_dgrad_graph->build_operation_graph(handle_comp_1_));
        ...
        if (search_mode_ == Config::SearchMode::EXHAUSTIVE_C) {
            ...
            if (exp_rec != nullptr) {
                ...
                auto [exp_status, matched] = match_and_build_plan(
                    conv_dgrad_graph, candidates, exp_rec, handle_comp_1_
                );
        ...
        fallback_heuristic_dgrad:
            ...
            CHECK_CUDNN_FE(conv_dgrad_graph->check_support(handle_comp_1_));
        ...
        // 单流模式：取最大workspace需求
        size_t dgrad_workspace = conv_dgrad_graph->get_workspace_size();
        workspace_size_comp_1_bwd_ = std::max(workspace_size_comp_1_bwd_, dgrad_workspace);
    }
    ...
    void build_conv_wgrad_graph() {
        ...
        CHECK_CUDNN_FE(conv_wgrad_graph->build_operation_graph(handle_comp_1_));
        ...
        if (search_mode_ == Config::SearchMode::EXHAUSTIVE_C) {
            ...
            if (exp_rec != nullptr) {
                ...
                auto [exp_status, matched] = match_and_build_plan(
                    conv_wgrad_graph, candidates, exp_rec, handle_comp_1_
                );
        ...
        fallback_heuristic_wgrad:
            ...
            CHECK_CUDNN_FE(conv_wgrad_graph->check_support(handle_comp_1_));
        ...
        // 单流模式：取最大workspace需求
        size_t wgrad_workspace = conv_wgrad_graph->get_workspace_size();
        workspace_size_comp_1_bwd_ = std::max(workspace_size_comp_1_bwd_, wgrad_workspace);
    }
...
public:
    ...
    void capture_cuda_graphs() {
        ...
        // === 单流顺序执行：BN Backward → WGrad → DGrad ===
        CHECK_CUDNN_FE(bn_bwd_graph->execute(handle_comp_1_, vp_bn_bwd_, d_workspace_comp_1_));
        CHECK_CUDNN_FE(conv_wgrad_graph->execute(handle_comp_1_, vp_wgrad_, d_workspace_comp_1_));
        CHECK_CUDNN_FE(conv_dgrad_graph->execute(handle_comp_1_, vp_dgrad_, d_workspace_comp_1_));
