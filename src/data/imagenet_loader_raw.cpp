...
void ImageNetLoaderRaw::begin_epoch(int epoch_id, bool is_train) {
    ...
    if (current_set_->mode == LoadMode::PARTIAL) {
        ...
        if (current_set_->full_arena == nullptr) {
            LOG_DEBUG << "FULLY mode: first epoch, allocating memory and loading buffers";
            ...
            // 准备buffer_metas数组（预分配，避免resize时的并发问题）
            ...
            // 5. 【关键修复】在epoch 0开始时预初始化thread_sample_info数组（避免多线程竞争）
    ...
    // 4. 初始化worker状态（所有模式、所有epoch都执行，保持与旧版本一致）
    for (int i = 0; i < num_preproc_workers_; ++i) {
        worker_states_[i].consuming_buffer = nullptr;
        worker_states_[i].local_idx = 0;
        worker_states_[i].global_seq = 0;  // 每个epoch开始时重置
...
void ImageNetLoaderRaw::end_epoch() {
    ...
    if (current_set_->mode == LoadMode::PARTIAL) {
        ...
        // 【关键修复】重置is_last_buffer标志，以便下一个epoch可以正常加载
        current_set_->is_last_buffer = false;
        // 【关键修复】重置cumulative_samples，以便下一个epoch可以重新开始计数
        current_set_->cumulative_samples = 0;
    } else {
...
void ImageNetLoaderRaw::merge_thread_samples_to_global(RawDataset& ds, bool is_train) {
    const int M = num_preproc_workers_;
...
void ImageNetLoaderRaw::shuffle_sample_info_array(std::vector<SampleInfo>& array, uint64_t seed) {
    /**
