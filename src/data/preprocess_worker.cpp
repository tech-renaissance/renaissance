...
PreprocessWorker::PreprocessWorker(
    ...
    : config_(config)
    , transfer_station_(config.transfer_station)  // 保存TransferStation指针
    , rng_(0)  // 临时seed，延迟初始化
{
    // =========================================================================
    ...
    // ==================== 从GlobalRegistry复制S区/C区单个样本紧凑字节数 ====================
    ...
    // 从GlobalRegistry复制AMP模式标志（构造时确定，永久使用）
    using_amp_ = GlobalRegistry::instance().using_amp();
...
uint8_t* PreprocessWorker::request_s_region_slot(int32_t label, int s_region_idx) {
    ...
    // ==================== 计算目标指针 ====================
    // 使用从GlobalRegistry复制的s_c_region_stride_（单个样本紧凑字节数）
...
uint8_t* PreprocessWorker::request_transfer_station_slot(int32_t label) {
    // ==================== 参数验证 ====================
    TR_CHECK(transfer_station_ != nullptr, ValueError,
             "transfer_station_ is null (test mode or not configured)");

    // ==================== 计算写入位置 ====================
    auto [batch_id, position] = calculate_write_position();

    // ==================== 请求TransferStation写入槽位 ====================
    // 零拷贝设计：直接返回TransferStation内部内存指针
...
bool PreprocessWorker::notify_transfer_station_sample_written() {
    // ==================== 调用TransferStation的notify方法 ====================
    // TransferStation会判断是否触发传输（batch满或其他条件）
    bool triggered = transfer_station_->notify_sample_written();
...
void PreprocessWorker::no_more_samples() {
    // ==================== 转发到TransferStation ====================
    // 使用pid_in_engine作为worker_id（TransferStation视角）
    transfer_station_->no_more_samples(config_.pid_in_engine);
}

// ============================================================================
// 从S区/C区复制样本到TransferStation
// ============================================================================
...
void PreprocessWorker::copy_sample_from_s_to_eb(int s_region_idx) {
    ...
    // ==================== 步骤3：计算TransferStation写入位置 ====================
    auto [batch_id, position] = calculate_write_position();

    // ==================== 步骤4：向TransferStation申请写入slot ====================
    uint8_t* eb_ptr = transfer_station_->request_write_slot(position, batch_id, label);

    // ==================== 步骤5：从S区复制图像数据 ====================
    // 计算当前resolution需要的字节数（考虑FusedNormalization的数据类型膨胀）
    int current_resolution = param_.is_train ? param_.current_train_resolution
                                              : param_.current_val_resolution;

    // 关键修正：必须使用与S区存储格式一致的字节数计算
    // S区存储的是FusedNormalization的输出，不是uint8_t数据
    // - FP32模式：current_resolution × current_resolution × num_channels × sizeof(float)
    // - FP16/AMP模式：current_resolution × current_resolution × 4 × sizeof(uint16_t)（固定4通道padding）
    // 使用构造时确定的using_amp_成员变量，避免每次调用GlobalRegistry
    size_t num_sample_bytes;

    if (using_amp_) {
        // FP16 AMP模式：固定4通道，每个元素2字节
        num_sample_bytes = static_cast<size_t>(current_resolution) * current_resolution * 4 * sizeof(uint16_t);
    } else {
        // FP32模式：原始通道数，每个元素4字节
        num_sample_bytes = static_cast<size_t>(current_resolution) * current_resolution * config_.num_color_channels * sizeof(float);
    }
    ...
    // ==================== 步骤6：通知TransferStation样本写入完成 ====================
    notify_transfer_station_sample_written();
}
...
void PreprocessWorker::copy_sample_from_c_to_eb() {
    ...
    // 关键修正：必须使用与C区存储格式一致的字节数计算
    // C区存储的是FusedNormalization的输出，不是uint8_t数据
    // - FP32模式：current_resolution × current_resolution × num_channels × sizeof(float)
    // - FP16/AMP模式：current_resolution × current_resolution × 4 × sizeof(uint16_t)（固定4通道padding）
    // 使用构造时确定的using_amp_成员变量，避免每次调用GlobalRegistry
    size_t num_sample_bytes;
...
std::pair<int, int> PreprocessWorker::calculate_write_position() const {
    // 核心公式（参考test_transfer_station_emulator.cpp第85-90行）
...
bool PreprocessWorker::work(
	...
	// 强制关闭RandomHorizontalFlip分支，因为翻转逻辑已融合到FusedNormalization内部
	// 即使GlobalRegistry中设置了train_with_rhf_/val_with_rhf_标志，这里也必须为false
	// 因为RandomHorizontalFlip已经从PO链中移除，不再作为独立操作存在
	const bool has_random_horizontal_flip = false;
	...
	for (int task_id = 0; task_id < num_preprocess_tasks; ++task_id) {
		...
		if (is_train) {
			if (task_id == num_preprocess_tasks - 1) {  // 最后一个任务，必定是输出到TransferStation
				final_output_ptr = request_transfer_station_slot(label);
			}
			else {
			...
			if (task_id == num_preprocess_tasks - 1) {  // 最后一个任务，必定是输出到TransferStation
				...
				if (task_id == 1) {
					// CPVS优化：从C区直接复制到TransferStation
					// C区和TransferStation都是紧凑布局，可以直接memcpy
					uint8_t* c_region_source_ptr = region_c_ptr_ + local_sample_id_ * s_c_region_stride_;
					size_t num_sample_bytes = using_amp_
						? static_cast<size_t>(res) * res * 4 * sizeof(uint16_t)
						: static_cast<size_t>(res) * res * num_color_channels * sizeof(float);
					std::memcpy(final_output_ptr, c_region_source_ptr, num_sample_bytes);
					break;  // 提前结束，本轮无需再预处理！
				}
			}
			else {
		...
		// =========================================================================
		// 执行所有PO（翻转/擦除/归一化已融合进FusedNormalization，无需外部优化分支）
		// =========================================================================
	...
	// ==================== 通知TransferStation样本写入完成 ====================
	// 【修复】NORMAL模式下，work()也需要通知TransferStation
...
void PreprocessWorker::work_lazy() {
    ...
    TR_CHECK(transfer_station_ != nullptr, ValueError,
             "transfer_station_ is null (lazy phase requires TransferStation)");
