...
EngineBuffer::EngineBuffer()
{
    ...
    current_sample_bytes_ = current_resolution_ * current_resolution_ * num_color_channels_;
}
...
#ifdef FOR_ENGINE_BUFFER_UNIT_TESTS_ONLY
// 在EngineBuffer的单元测试中，由于没有深度学习引擎取用数据并将标志设为可写，所以必须自动设为立即可写，否则会一直卡死
    set_buffer_writeable(buf_id, true);
// 但在单元测试以外，这句话不可以有，否则就会破坏buffer中暂未读取完成的数据
#endif  // FOR_ENGINE_BUFFER_UNIT_TESTS_ONLY
...
#ifdef ENGINE_BUFFER_SAVE_FIRST_IMAGE
void EngineBuffer::save_first_image() {
    const size_t snapshot_sample_bytes = current_sample_bytes_;
