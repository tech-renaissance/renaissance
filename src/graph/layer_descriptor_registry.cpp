...
// ============================================================================
// ChannelPadding — 1张量（C通道填充到8的倍数，尾部补零）
// ============================================================================

std::vector<TensorDesc> infer_channel_padding_tensors(
    const Shape& input, const OpParams&, const InferContext& ctx)
{
    DType feat_dt = ctx.enable_amp ? DType::FP16 : DType::FP32;
    int out_c = (input.c() + 7) / 8 * 8;
    return {
        TensorDesc{"channel_padding_output",
                   Shape{input.n(), input.h(), input.w(), out_c},
                   select_feature_region(ctx), feat_dt}
    };
}

SubgraphPattern build_channel_padding_forward(const OpParams&, const std::vector<TensorDesc>& descs) {
    SubgraphPattern p;
    if (descs.empty()) return p;
    SubgraphPattern::Node n;
    n.op = GlobalRegistry::instance().using_amp() ? ComputeOp::CHANNEL_PADDING_AMP_FWD
                                                   : ComputeOp::CHANNEL_PADDING_FP32_FWD;
    n.output_indices = {0};
    p.nodes.push_back(n);
    return p;
}

SubgraphPattern build_channel_padding_backward(const OpParams&, const std::vector<TensorDesc>& descs) {
    SubgraphPattern p;
    if (descs.empty()) return p;
    SubgraphPattern::Node n;
...
SubgraphPattern build_channel_padding_inference(const OpParams&, const std::vector<TensorDesc>& descs) {
    return build_channel_padding_forward({}, descs);
}
...
Shape get_output_shape(LayerKind kind, const std::vector<TensorDesc>& descs)
{
    ...
    switch (kind) {
        ...
        case LayerKind::ChannelPadding: return find(0);   // output at index 0
...
const LayerDescriptor& get_layer_descriptor(LayerKind kind) {
    ...
    static const LayerDescriptor chpad_desc  = { infer_channel_padding_tensors, build_channel_padding_forward, build_channel_padding_backward, build_channel_padding_inference };
    ...
    switch (kind) {
        ...
        case LayerKind::ChannelPadding:       return chpad_desc;
