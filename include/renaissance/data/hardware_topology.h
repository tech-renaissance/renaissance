...
// CUDA头文件（discover_gpus需要）
#if defined(TR_USE_CUDA)
    #include <cuda_runtime.h>
#endif

// ============================================================================
// 仅在GPU_CLOUD场景下启用硬件拓扑检测
// ============================================================================
...
struct PhysicalCore {
    int core_id;                  ///< 物理核ID
    std::vector<int> logic_cpus;  ///< 该物理核的逻辑线程（超线程）
};
...
struct NumaNode {
    ...
    std::vector<PhysicalCore> physical_cores; ///< 物理核视图
...
struct GpuDevice {
    int id;                  ///< GPU ID
    int pci_bus;             ///< PCI总线ID
    int pci_domain;          ///< PCI域ID
    int numa_node;           ///< NUMA节点ID
    std::string pci_dbdf;    ///< PCI DBDF标识
    std::vector<int> affinity_cpus;  ///< GPU本地CPU列表
};
...
private:
    ...
    std::map<int, NumaNode> nodes_;    ///< node_id -> NumaNode
    std::vector<GpuDevice> gpus_;     ///< GPU设备列表
    int num_logical_cpus_ = 0;          ///< 逻辑CPU总数
};

// ============================================================================
// CpuBindingPlanner - CPU绑核规划器
// ============================================================================
...
public:
    explicit CpuBindingPlanner(const HardwareTopology& topo) : topo_(topo) {}
