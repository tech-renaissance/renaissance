...
public:
    ...
    struct Config {
        ...
        // ==================== 测试模式 ====================
        bool test_mode = false;      ///< 测试模式（不需要TransferStation）

        // ==================== TransferStation（V3.14.0）====================
        TransferStation* transfer_station = nullptr;  ///< TransferStation指针（不拥有所有权，仅观察）
     ...
     *
     * 设计原则：
     * - 与TransferStation对齐：获取指针的同时写入标签
    ...
    /**
     * @brief 请求TransferStation写入槽位（获取指针并保存标签）
     * @param label 样本标签
     * @return TransferStation数据写入位置的指针
     *
     * 设计原则：
     ...
     * 2. 调用TransferStation::request_write_slot()获取指针并保存标签
     * 3. 返回指针供PW直接写入数据
     *
     * 注意事项：
     * - 此方法可能阻塞（批次边界保护）
     * - 返回指针生命周期：直到下一次notify_transfer_station_sample_written()
     */
    uint8_t* request_transfer_station_slot(int32_t label);

    /**
     * @brief 通知TransferStation一个样本写入完成
     ...
     * 2. 判断是否是该PW的最后一个样本
     * 3. 调用TransferStation::notify_sample_written()
     * 4. 递增local_sample_id_
     */
    bool notify_transfer_station_sample_written();

    /**
     * @brief 通知TransferStation该PW没有更多样本
     *
     * 设计原则：
     * - PW在获取不到更多样本时调用（DataLoader返回false）
     * - 直接转发到TransferStation::no_more_samples()
    ...
    /**
     * @brief 从S区复制样本到TransferStation（Lazy phase使用）
     * @param s_region_idx S区索引（0 ~ num_region_s-1）
     *
     * 工作流程：
     * 1. 根据local_sample_id_从s_shuffled_indices_查询槽位编号
     * 2. 从对应的s_label_vectors_获取标签
     * 3. 计算TransferStation写入位置（调用calculate_write_position）
     * 4. 向TransferStation申请写入slot（调用request_transfer_station_slot）
     * 5. 从S区复制图像数据（只复制当前resolution需要的字节数）
     * 6. 通知TransferStation样本写入完成（调用notify_transfer_station_sample_written）
     *
     * 注意：
    ...
    /**
     * @brief 从C区复制样本到TransferStation（Lazy val phase使用）
     *
     * 工作流程：
     ...
     * - 需要TransferStation
     * - 根据is_train/val判断：
     *   - Busy train phase: 写入S区（前sdmp_factor-1次）或TransferStation（最后一次）
     *   - Busy val phase: 写入C区（如果启用CPVS）或TransferStation
     *
     * 注意：
     * - 此方法只在busy phase调用
     * - 暂未实现（TODO）
...
private:
    ...
    TransferStation* transfer_station_ = nullptr;  ///< TransferStation指针（不拥有所有权，由Preprocessor管理）
    ...
    bool using_amp_ = false;             ///< 是否使用AMP模式（从GlobalRegistry复制，构造时确定）
     ...
     * @note 在解码、预处理、存储到S/C区、保存到TransferStation完成后调用
     */
    void end_sample();
    ...
    /**
     * @brief 计算在TransferStation中的写入位置（零竞争设计）
     * @return std::pair<int, int> {batch_id, position}
     *
     * 核心公式（参考test_transfer_station_emulator.cpp）：
